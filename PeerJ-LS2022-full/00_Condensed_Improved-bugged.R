...
colnames(data) <- c('ID', 'Start_time', 'Completion_time', 'Email', 'Name', 'Consent_read',
                    'Experience_yrs', 'Gender', '01_01', '01_02', '01_03', '01_04', '01_05',
                    '01_06', '01_07', '01_08', '01_09', '01_10', '03_01', '03_02', '03_03', 
                    '03_04', '03_05', '03_06', '03_07', '03_Role', '04_01', '04_02', 
                    '04_03', '04_04', '04_05', '04_06', '04_07', '04_Role', '05_01', 
                    '05_02', '05_03', '05_04', '05_05', '05_06', '05_07', '05_Role', 
                    '06_01', '06_02', '06_03', '06_04', '06_05', '06_06', '06_07', '06_Role', 
                    '07_01', '07_02', '07_03', '07_04', '07_05', '07_06', '07_07', '07_Role',
                    '08_01', '08_02', '08_03', '08_04', '08_05', '08_06', '08_07', '08_Role',
...
data[9:66] <- lapply(colnames(data)[9:66], function(col_name) {
  if (grepl("Role", col_name)) {
    return(data[[col_name]])  # Skip Role columns
  } else {
...
# Convert columns 9 to 66 to numeric
data[9:66] <- lapply(data[9:66], as.numeric)

# Clean initials and generate student IDs
data$Initials <- tolower(substring(data$Initials, nchar(data$Initials) - 5))
data$Student_ID <- openssl::md5(data$Initials)

# Create cleaned dataset with Big Five personality traits and intrinsic motivation per round
Stats <- data.frame(
  Start_time = as.Date(data$Start_time, origin = "1899-12-30"),
  Completion_time = data$Completion_time,
  Experience_yrs = data$Experience_yrs,
  Gender = data$Gender,
  Student_ID = data$Student_ID,
  Initials = data$Initials,
  Exc_round = ceiling((as.numeric(as.Date(data$Start_time)) - 44652) / 7),
  
  # Use last session for BFI-10 traits
  B5_O = (6 - data$`01_05`) + data$`01_10`,
  B5_C = (6 - data$`01_03`) + data$`01_08`,
  B5_E = (6 - data$`01_01`) + data$`01_06`,
  B5_A = data$`01_02` + (6 - data$`01_07`),
  B5_N = (6 - data$`01_04`) + data$`01_09`,
...
# Use last session's BFI-10 data
last_session_data <- Stats %>%
  group_by(Initials) %>%
  filter(Exc_round == max(Exc_round))

# Continue selecting last session's BFI-10 data
clust_data <- last_session_data %>%
  select(Initials, B5_O, B5_C, B5_E, B5_A, B5_N)
...
# Ensure that Initials are lowercase and trimmed in both datasets
Stats$Initials <- tolower(trimws(Stats$Initials))
clust_data$Initials <- tolower(trimws(clust_data$Initials))

# Add Cluster column to Stats by joining with clust_data
...
Stats_clustered <- Stats_clustered %>%
