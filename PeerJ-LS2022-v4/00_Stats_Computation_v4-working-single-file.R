...
# Step 1: Create Stats_Session_Data.xlsx
...
write.xlsx(Stats_Session_Data, file = "Stats_Session_Data.xlsx", colNames = TRUE, overwrite = TRUE)

# Step 2: Create Stats_B5Avg.xlsx
...
write.xlsx(Stats_B5Avg, file = "Stats_B5Avg.xlsx", colNames = TRUE, overwrite = TRUE)

# Step 3: Create Stats_Ready.xlsx
# Iterate through each participant in Stats_B5Avg
for (i in 1:nrow(Stats_B5Avg)) {
  # Extract the current participant's initials
  current_initials <- Stats_B5Avg$Initials[i]
  
  # Extract the averaged Big Five scores for the current participant
  avg_B5 <- Stats_B5Avg[i, c("B5_O", "B5_C", "B5_E", "B5_A", "B5_N")]
  
  # Replace Big Five scores in Stats_Session_Data for all matching Initials
  Stats_Session_Data[Stats_Session_Data$Initials == current_initials, 
                     c("B5_O", "B5_C", "B5_E", "B5_A", "B5_N")] <- avg_B5
}

# Save the updated dataset to Stats_Ready.xlsx
write.xlsx(Stats_Session_Data, file = "Stats_Ready.xlsx", colNames = TRUE, overwrite = TRUE)
