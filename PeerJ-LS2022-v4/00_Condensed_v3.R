...
# It is further enhanced by scripts 01, 02, and 03.
...
# Read data
data <- read.xlsx("Experiment-PeerJ-SS2022.xlsx", sheet = "Sheet1")
...
write.xlsx(
  Stats[, !(names(Stats) %in% c("Start_time", "Completion_time", "Gender", "Experience_yrs", "Ps_results_flag"))], 
...
# Create separate data frames for each pair of roles
# Ensure each participant has data for all roles in pairwise comparisons
pilot_nav <- motivation_by_role %>% 
  filter(Role == "Pilot" | Role == "Navigator") %>%
  group_by(Initials) %>% 
  filter(n() == 2)  # Only keep participants with both roles

pilot_solo <- motivation_by_role %>% 
  filter(Role == "Pilot" | Role == "Solo") %>%
  group_by(Initials) %>% 
  filter(n() == 2)  # Only keep participants with both roles

nav_solo <- motivation_by_role %>% 
  filter(Role == "Navigator" | Role == "Solo") %>%
  group_by(Initials) %>% 
  filter(n() == 2)  # Only keep participants with both roles

# Wilcoxon signed-rank test for Pilot vs. Navigator (handling ties)
wilcox_pilot_nav <- wilcox.test(
  pilot_nav$mean_motivation[pilot_nav$Role == "Pilot"], 
  pilot_nav$mean_motivation[pilot_nav$Role == "Navigator"], 
  paired = TRUE, 
  alternative = "two.sided"
)
# Wilcoxon signed-rank test for Pilot vs. Navigator
wilcox_pilot_nav <- wilcox.test(
  pilot_nav$mean_motivation[pilot_nav$Role == "Pilot"], 
  pilot_nav$mean_motivation[pilot_nav$Role == "Navigator"], 
  paired = TRUE, alternative = "two.sided"
)
# Wilcoxon signed-rank test for Pilot vs. Solo
wilcox_pilot_solo <- wilcox.test(
  pilot_solo$mean_motivation[pilot_solo$Role == "Pilot"], 
  pilot_solo$mean_motivation[pilot_solo$Role == "Solo"], 
  paired = TRUE, alternative = "two.sided"
)
# Wilcoxon signed-rank test for Navigator vs. Solo
wilcox_nav_solo <- wilcox.test(
  nav_solo$mean_motivation[nav_solo$Role == "Navigator"], 
  nav_solo$mean_motivation[nav_solo$Role == "Solo"], 
  paired = TRUE, alternative = "two.sided"
)

# Calculate effect sizes (r) for each pairwise comparison
# r = Z / sqrt(N) where Z is the Wilcoxon test statistic and N is the number of observations
effect_size_pilot_nav <- wilcox_pilot_nav$statistic / sqrt(length(pilot_nav$mean_motivation[pilot_nav$Role == "Pilot"]))
effect_size_pilot_solo <- wilcox_pilot_solo$statistic / sqrt(length(pilot_solo$mean_motivation[pilot_solo$Role == "Pilot"]))
effect_size_nav_solo <- wilcox_nav_solo$statistic / sqrt(length(nav_solo$mean_motivation[nav_solo$Role == "Navigator"]))

# Display results
list(
  wilcox_pilot_nav = wilcox_pilot_nav,
  effect_size_pilot_nav = effect_size_pilot_nav,
  
  wilcox_pilot_solo = wilcox_pilot_solo,
  effect_size_pilot_solo = effect_size_pilot_solo,
  
  wilcox_nav_solo = wilcox_nav_solo,
  effect_size_nav_solo = effect_size_nav_solo
)

# Descriptive - Identify the anonymized individual with the highest percentual
# average increase across roles.
# Step 2: Calculate the percentual increase
percentual_increase <- motivation_by_role %>%
  group_by(Initials) %>%
  summarize(
    max_motivation = max(mean_motivation, na.rm = TRUE),
    min_motivation = min(mean_motivation, na.rm = TRUE),
    percentual_increase = ((max_motivation - min_motivation) / min_motivation) * 100
  )

# Step 3: Identify the individual with the largest percentual increase
individual_largest_increase <- percentual_increase %>%
  arrange(desc(percentual_increase)) %>%
  slice(1)

# Step 4: Display results
individual_largest_increase
...
# ### Role Preference Contingency on Cluster ###
chisq.test(HD, pref$max_mean, correct=FALSE)
chisq.test(HD, retRole(person_res,cols,"max"), correct=FALSE)

fisher.test(HD, pref$max_mean)
chisq.test(HD, pref$max_mean, simulate.p.value = TRUE)
