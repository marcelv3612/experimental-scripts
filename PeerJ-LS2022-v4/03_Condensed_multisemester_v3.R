# 03_Condensed_Multisem_v4.R
# PeerJ-CS (SS2022 + WS2021), "Personality-Based Pair Programming"
...
# Update the role mapping function to handle variations in role responses
role_mapping <- function(x) {
  x <- trimws(tolower(x))  # Standardize case and remove leading/trailing whitespaces
  ifelse(x == "disagree strongly", "Pilot",
         ifelse(x == "neither agree nor disagree", "Solo", 
                ifelse(x == "agree strongly", "Navigator", NA)))
}
...
# Filter rows with valid Start_time
data <- data[!is.na(data$Start_time), ]

# Convert relevant columns to numeric
data[9:66] <- lapply(data[9:66], as.numeric)

# Clean initials (remove suffixes) and hash ids to create unique Student_ID
data$Initials <- tolower(substring(data$Initials, nchar(data$Initials) - 5))
data$Student_ID <- md5(data$Initials)

# Create experiment round variable based on Start_time
data$Start_time <- as.Date(data$Start_time, origin = "1899-12-30")  # Convert to date format
data$Exc_round <- ceiling((as.numeric(data$Start_time) - 44652) / 7)
data$Exc_round <- ifelse(data$Exc_round == 7, 3, data$Exc_round)
...
data <- data %>%
  mutate(
    B5_O = ((6 - `01_05`) + `01_10`) / 2,
    B5_C = ((6 - `01_03`) + `01_08`) / 2,
    B5_E = ((6 - `01_01`) + `01_06`) / 2,
    B5_A = (`01_02` + (6 - `01_07`)) / 2,
    B5_N = ((6 - `01_04`) + `01_09`) / 2
  )

# Filter rows with complete Big Five data
data <- data[complete.cases(data[c("B5_O", "B5_C", "B5_E", "B5_A", "B5_N")]), ]

# Group by Student_ID and calculate averaged Big Five scores and intrinsic motivation
averaged_data <- data %>%
  group_by(Student_ID) %>%
  summarize(
    Initials = `Initials`,
    ...
    INNER_R1 = round(mean(`03_01` + `03_02` + (6 - `03_03`) + (6 - `03_04`) + `03_05` + `03_06` + `03_07`, na.rm = TRUE), 4),
    INNER_R2 = round(mean(`04_01` + `04_02` + (6 - `04_03`) + (6 - `04_04`) + `04_05` + `04_06` + `04_07`, na.rm = TRUE), 4),
    INNER_R3 = round(mean(`05_01` + `05_02` + (6 - `05_03`) + (6 - `05_04`) + `05_05` + `05_06` + `05_07`, na.rm = TRUE), 4),
    INNER_R4 = round(mean(`06_01` + `06_02` + (6 - `06_03`) + (6 - `06_04`) + `06_05` + `06_06` + `06_07`, na.rm = TRUE), 4),
    INNER_R5 = round(mean(`07_01` + `07_02` + (6 - `07_03`) + (6 - `07_04`) + `07_05` + `07_06` + `07_07`, na.rm = TRUE), 4),
    INNER_R6 = round(mean(`08_01` + `08_02` + (6 - `08_03`) + (6 - `08_04`) + `08_05` + `08_06` + `08_07`, na.rm = TRUE), 4),
    .groups = "drop"
  )

# Save the averaged data
write.xlsx(averaged_data, file = "Stats_Multisem_Averaged.xlsx", colNames = TRUE, overwrite = TRUE)
...
write.xlsx(Stats, file = "Stats_Multisem.xlsx", colNames = TRUE, overwrite = TRUE)

# Group by Student_ID and calculate the averages of the Big Five traits across sessions, rounded to 4 decimal places
averaged_data <- data %>%
  group_by(Student_ID) %>%
  mutate(
    B5_O = round(mean(B5_O, na.rm = TRUE), 4),
...
# Save the updated dataset with averaged Big Five traits
write.xlsx(averaged_data, file = "Stats_Multisem_Averaged.xlsx", colNames = TRUE, overwrite = TRUE)
...
#### Testing the individual differences in motivation by role
